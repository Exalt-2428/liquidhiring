...
const JobDetails = ({ job, candidates, access_token }) => {
  ...
  }, [error, job, access_token]);

  const applyToJobHandler = () => {
    try {
      applyToJob(job.id, access_token);
      toast.success("Job applied successfully", { toastId: "jobApplied" });
    }
    catch (error) {
      toast.error(error, { toastId: "jobApplied" });
      clearErrors();
    }
  };

  const alreadyAppliedHandler = () => {
    toast.error("You have already applied to this job", {
      toastId: "alreadyApplied",
    });
  };
  const d1 = moment(job.lastDate);
  const d2 = moment(Date.now());
  const isLastDatePassed = d1.diff(d2, "days") < 0 ? true : false;
  console.log(d1, d2, isLastDatePassed);
  return (
                ...
                <div className="mt-3">
                  <span>
                    {loading ? (
                      ...
                      <button
                        className="btn btn-success px-4 py-2 apply-btn disabled" onClick={alreadyAppliedHandler}
                      >
            ...
            {isLastDatePassed && (
              <div className="mt-5 p-0">
                <div className="alert alert-danger">
                  ...
                  <br /> Checkout others job on LiquidHiring.
                </div>
              </div>
            )}
          </div>
